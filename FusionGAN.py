...
class FusionGAN(object) :
    ...
    def generator(self, x_identity, x_shape, reuse=False, scope="generator"):
        ...
        """
        with tf.variable_scope(scope, reuse=reuse):
            contents, shape_list = self.Content_Encoder(x_shape, reuse=reuse)
            style, identity_list = self.Style_Encoder(x_identity, reuse=reuse)
            ...
            for i in range(self.n_upsample) :
                # # IN removes the original feature mean and variance that represent important style information
                x = concat([x, identity_list[1-i], shape_list[1-i]])
    ...
    def build_model(self):
        ...
        """ Define Loss """
        g_identity_loss = self.adv_weight * generator_loss(self.gan_type, fake_logit) * 64
        g_shape_loss_same = self.L1_weight * L1_loss(self.fake_same, self.shape_A)
        g_shape_loss_diff_shape = self.L1_weight * L1_loss(fake_diff_shape, self.shape_B) + self.L1_weight * L1_loss(self.fake_diff, self.shape_B)
...
#            ckpt_name = "FusionGAN.model-29001"
